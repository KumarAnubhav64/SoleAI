...
export function SafetyInstructions({ equipmentType, severity }: SafetyInstructionsProps) {
  const instructions = (safetyData as Record<string, Record<string, string[]>>)[equipmentType]?.[
    severity
  ];
  ...
  return (
    <div
      className={`rounded-xl border p-5 ${
        isCritical ? 'border-red-500/30 bg-red-500/5' : 'border-slate-700 bg-slate-900'
      }`}
    >
      {/* Header */}
      <div className="mb-4 flex items-center gap-3">
        ...
        <div>
          <h3 className="text-sm font-semibold text-slate-100">Safety Instructions</h3>
            ...
            <li key={index} className="flex items-start gap-2.5">
              <span
                className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ${
                  isUrgent ? 'bg-red-500/20 text-red-400' : 'bg-slate-800 text-slate-500'
                }`}
              >
                {index + 1}
              </span>
              <span
                className={`text-xs leading-relaxed ${
                  isUrgent ? 'font-medium text-red-300' : 'text-slate-400'
                }`}
              >
                {instruction}
              </span>
